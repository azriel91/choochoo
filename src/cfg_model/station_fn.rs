...
pub type StationFnReturn<'f, E> = Pin<Box<dyn Future<Output = Result<(), E>> + Send + Sync + 'f>>;

/// Steps to run for this part of the station's logic.
pub struct StationFn<E>(
    pub Arc<dyn Fn(&'_ mut Station<E>) -> StationFnReturn<'_, E> + Send + Sync>,
);
...
impl<E> StationFn<E> {
    ...
    where
        F: Fn(&'_ mut Station<E>) -> StationFnReturn<'_, E> + Send + Sync + 'static,
    {
        Self(Arc::new(f))
    }
}
...
impl<E> Clone for StationFn<E> {
    fn clone(&self) -> Self {
        Self(Arc::clone(&self.0))
    }
}

impl<E> Debug for StationFn<E> {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        f.write_str("StationFn(fn(&'_ mut Station<E>) -> StationFnReturn<'_, E>)")
    }
}

impl<E> PartialEq for StationFn<E> {
...
mod tests {
    ...
    fn debug_impl_includes_all_fields() {
        ...
        assert_eq!(
            "StationFn(fn(&'_ mut Station<E>) -> StationFnReturn<'_, E>)",
            format!("{:?}", visit_fn)
        );
    }
